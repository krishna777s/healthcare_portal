...
import { useIcuAlerts, useDoctorStats } from "@/hooks/useDoctorData";

const navigation = [
  { name: "Overview", href: "/dashboard", icon: LayoutDashboard },
  { name: "Outpatients", href: "/dashboard/my-patients", icon: Users },
  { name: "Inpatients", href: "/dashboard/inpatients", icon: BedDouble },
  { name: "ICU", href: "/dashboard/icu", icon: Activity, badge: "icu" },
...
export const DoctorSidebar = () => {
  ...
  const { data: alerts = [] } = useIcuAlerts();

  const unreadAlerts = alerts.filter((a: any) => !a.is_acknowledged).length;
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3">
              ...
              {isOpen && (
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-[#D1D5DB]">Doctor Portal</span>
                </div>
              )}
            </div>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto py-6">
            <ul className="space-y-1 px-3">
              {navigation.map((item) => {
                const badgeCount = item.badge === "icu" ? unreadAlerts : 0;
                return (
                  <li key={item.name}>
                    <NavLink
                      to={item.href}
                      ...
                      className={({ isActive }) =>
                        cn(
                          ...
                          isActive
                            ...
                            : item.name === "ICU" ? "text-red-300 hover:text-white" : "text-white"
                        )
                      }
                    >
                      ...
                      {!isOpen && badgeCount > 0 && (
                        <span className="absolute left-12 h-4 w-4 flex items-center justify-center rounded-full bg-red-500 text-white text-xs font-bold">
                          {badgeCount}
                        </span>
                      )}
                    </NavLink>
